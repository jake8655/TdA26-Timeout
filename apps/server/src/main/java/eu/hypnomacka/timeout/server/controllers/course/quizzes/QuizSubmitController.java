...
public class QuizSubmitController extends Controller {
    ...
    public ResponseEntity<?> submitQuiz(
                ...
                if (answer.getSelectedIndex() == null) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body(
                                    Map.of(
                                            "message",
                                            "selectedIndex is required for singleChoice question"));
                }
                ...
                if (answer.getSelectedIndices() == null) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body(
                                    Map.of(
                                            "message",
                                            "selectedIndices is required for multipleChoice"
                                                + " question"));
                }
        ...
        QuizResult result =
                new QuizResult(quiz, score, maxScore, correctPerQuestion, Instant.now());
        result.save();
